...
router.get('/guilds/:guildID', async (req, res, next) => {
    ...
    const { status, statusColor } = getBotStatus(bot);
    const guild = guilds.find(g => g.id === req.params.guildID && bot.guilds.has(g.id));
    const guildChannels = bot.guilds.get(guild.id).channels;
    const textChannels = guildChannels.filter(ch => ch.type === 'text').sort((a, b) => (a.position < b.position ? -1 : 1));
    const categories = guildChannels.filter(ch => ch.type === 'category').sort((a, b) => (a.position < b.position ? -1 : 1));
