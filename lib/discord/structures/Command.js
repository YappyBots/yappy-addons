...
class Command {
    ...
    /**
     * Convert command data into Discord SlashCommand
     */
    getSlashCommand() {
        const { name, summary, description } = this.props.help;

        try {
            const command = new SlashCommandBuilder().setName(name).setDescription(summary || description);

            if (this.conf.permLevel >= 1) {
                command.setDefaultMemberPermissions(PermissionFlagsBits.Administrator);
            }

            if (this.conf.permLevel >= 1 || this.conf.guildOnly) {
                command.setDMPermission(false);
            }

            return command;
        } catch (err) {
            Log.error(`Failed to convert '${name}' info to SlashCommand`);
    ...
    commandError(msg, err, title, footer) {
        ...
        const payload = { embeds: [embed], ephemeral: !!this.conf.ephemeral };

        if (msg.editReply) {
            if (msg.deferred || msg.replied) {
                return msg.editReply(payload);
            }
    ...
    textToEmbed(title = 'Auto Generated Response', text, color = '#84F139') {
        ...
        return new this.embed().setColor(color).setTitle(title).setDescription(text).setFooter({
    ...
    _permLevelToWord(permLvl) {
        ...
        else if (permLvl === 1) return 'Administrator';
        else if (permLvl === 2) return 'Bot Owner';
        else return '???';
    }
}
